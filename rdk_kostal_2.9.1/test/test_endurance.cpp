...
int main(int argc, char* argv[])
{
    ...
    double testHours = std::stof("0.001");
    ...
    try {
        ...
        log.info("The robot is in plan execution mode");
        robot.executePlanByName("PLAN-MoveCircle"); //PLAN-MoveCircle
        ...
        log.info("The execution is begin");
        // wait until execution finished
        do {
            robot.getSystemStatus(&systemStatus);
            std::this_thread::sleep_for(std::chrono::milliseconds(1));
        } while (systemStatus.m_programRunning);
        log.info("The execution is finished");
        ...
        scheduler.addTask(std::bind(highPriorityTask, &robot, &robotStates, &scheduler, &log), "HP periodic", 1, 45);
